...
While it would be easier to export a CSV file from the Excel spreadsheet and then [import that CSV file into the Collection](../../processmaker-collections/manage-records-in-a-collection/create-multiple-collection-records-from-a-csv-file.md) to automatically create multiple records, this use case and its accompanying ProcessMaker assets benefits those organizations where business stakeholders do not have access to ProcessMaker Collections or the appropriate permissions to edit them. Any ProcessMaker user may benefit from this use case.
